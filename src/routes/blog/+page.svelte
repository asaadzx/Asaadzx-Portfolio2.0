...
<section class="w-full max-w-4xl mx-auto mt-16 px-6 font-body text-text">
	<div
		class="mb-10 border-b border-text/10 pb-4 flex justify-between items-end text-xs opacity-50"
	>
		<div>
			<p class="font-bold text-primary">[ CMD: query_archive_logs --all ]</p>
			<p class="text-[11px] mt-1">FOUND: {data.posts.length} INDEX_NODES IN DIRECTORY</p>
		</div>
	...
	<div class="space-y-1">
		{#each data.posts as post, index (post.slug)}
			<div bind:this={postRows[index]} class="opacity-0 will-change-transform">
				<a
				...
				>
					<div class="flex items-start sm:items-center gap-4 z-10">
						<span class="text-[11px] font-mono opacity-30 mt-0.5 sm:mt-0">
							#{String(data.posts.length - index).padStart(2, '0')}
						</span>
						<div class="space-y-1">
							<h2
							...
							<p class="text-xs text-text/50 line-clamp-1 max-w-xl">
								{post.meta.excerpt}
							</p>
						</div>
					</div>
